/**
 * QMC 密钥派生模块
 * 浏览器兼容版：使用 Uint8Array 替代 Node.js Buffer
 */
...
export function QmcDeriveKey(raw: Uint8Array): Uint8Array {
  ...
  const decoded = textDec.decode(raw);
  let rawDec = base64ToUint8Array(decoded);
  const n = rawDec.length;
  if (n < 16) {
...
function decryptV2Key(key: Uint8Array): Uint8Array {
  const textDec = new TextDecoder();
  if (key.length < 18 || textDec.decode(key.slice(0, 18)) !== 'QQMusic EncV2,Key:') {
    return key;
  }

  let out = decryptTencentTea(key.slice(18), mixKey1);
  out = decryptTencentTea(out, mixKey2);
  const keyDec = base64ToUint8Array(textDec.decode(out));
